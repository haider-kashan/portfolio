      ...
      {Array.from(groupedSkills.entries()).map(([category, categorySkills]) => {
        ...
        return (
          ...
          >
            ...
            {/* 1. VISUAL CHART (Hidden from Screen Readers to avoid duplication) */}
            ...
            {/* 2. SEO & ACCESSIBILITY TABLE (Visually Hidden) */}
            {/* This provides perfect structured data for search engines */}
            <div className="sr-only">
              <table>
                <caption>{displayLabel} Skills Proficiency Levels</caption>
                <thead>
                  <tr>
                    <th scope="col">Skill Name</th>
                    <th scope="col">Proficiency Percentage</th>
                  </tr>
                </thead>
                <tbody>
                  {categorySkills.map((skill, index) => (
                    <tr key={`${category}-row-${index}`}>
                      <td>{skill.name}</td>
